...
public class GameWindow extends JFrame implements ActionListener, KeyListener {
	...
	public void rotateRight() {
		...
		// We will use this method to generalize right Rotation for all tetroids apart
		// from the "I" tetroid
		...
		switch (state) {
		case 0:
			Block[][] temp = startRight(rotategrid);
			if (temp == rotategrid) {
				// rotation failed
				panelField = restore;
				break;
			...
			for (int r = 0; r < 3; r++) {
				for (int c = 0; c < 3; c++) {
					if (currentElement.contains(temp[r][c]) && temp[r][c].isMarked() == false) {
						...
						// And update the panel Field by adding the new coordinates (which are null as
						// asserted by our rotation method)...
						Block b = temp[r][c];
						...
						// Remove the Element from currentElement until we finish rotating all the other
						// blocks
						b.setMarked(true);
		...
		case 1:
			Block[][] temp1 = startBottom(rotategrid);
			if (temp1 == rotategrid) {
				// rotation failed
				panelField = restore;
				break;
			...
			for (int r = 0; r < 3; r++) {
				for (int c = 0; c < 3; c++) {
					if (currentElement.contains(temp1[r][c]) && temp1[r][c].isMarked() == false) {
						// Old coordinates for deletion
						int deleter = temp1[r][c].getRow();
						int deletec = temp1[r][c].getCol();

						// Input the new coordinates
						temp1[r][c].setRow(rotator.getRow() - 1 + r);
						temp1[r][c].setCol(rotator.getCol() - 1 + c);
		...
		case 2:
			Block[][] temp2 = startLeft(rotategrid);
			if (temp2 == rotategrid) {
				// rotation failed
				panelField = restore;
				break;
			...
			for (int r = 0; r < 3; r++) {
				for (int c = 0; c < 3; c++) {
					if (currentElement.contains(temp2[r][c]) && temp2[r][c].isMarked() == false) {
						// Old coordinates for deletion
						int deleter = temp2[r][c].getRow();
						int deletec = temp2[r][c].getCol();

						// Input the new coordinates
						temp2[r][c].setRow(rotator.getRow() - 1 + r);
						temp2[r][c].setCol(rotator.getCol() - 1 + c);
		...
		case 3:
			Block[][] temp3 = startLeft(rotategrid);
			if (temp3 == rotategrid) {
				// rotation failed
				panelField = restore;
				break;
			...
			for (int r = 0; r < 3; r++) {
				for (int c = 0; c < 3; c++) {
					if (currentElement.contains(temp3[r][c]) && temp3[r][c].isMarked() == false) {
						// Old coordinates for deletion
						int deleter = temp3[r][c].getRow();
						int deletec = temp3[r][c].getCol();

						// Input the new coordinates
						temp3[r][c].setRow(rotator.getRow() - 1 + r);
						temp3[r][c].setCol(rotator.getCol() - 1 + c);
	...
	public Block[][] startTop(Block[][] input) {
		Block[][] temp = input;

		// Backup Block if rotation fails
		Block[][] restore = input;

		// Go from Left to Right and then bottom to top rotate all the elements. This
		// algorithm has been emperically tested and works for all blocks that are
		// rotated 180 degrees
		...
		for (int r = 0; r < 3; r++) {
			for (int c = 0; c < 3; c++) {
				if (currentElement.contains(temp[r][c]) && temp[r][c].isMarked() == false) {
					...
					// Through practical tests or a simple check one can see that with this
					// Algorithm used on tetroids in the rotated right once position no feel that a
					// block gets copied to
					// Is allowed to be null, hence we can practically assert whether or not a
					// rotation is possible or not.
					if (temp[c][2 - r] != null) {
						temp = restore;
						return temp;
					}
	...
	public Block[][] startLeft(Block[][] input) {
		Block[][] temp = input;
		...
		for (int c = 0; c < 3; c++) {
			for (int r = 0; r < 3; r++) {
				if (currentElement.contains(temp[2 - r][c]) && temp[2 - r][c].isMarked() == false) {
					...
					// Is allowed to be null, hence we can practically assert whether or not a
					// rotation is possible or not.
					if (temp[c][2 - (2 - r)] != null) {
						temp = restore;
						return temp;
					}

					temp[c][2 - (2 - r)] = temp[2 - r][c];
					temp[2 - r][c] = null;
					temp[c][2 - (2 - r)].setMarked(true);
				}
			}
		}
		for (Block block : currentElement) {
	...
	public Block[][] startBottom(Block[][] input) {
		Block[][] temp = input;

		// Backup Block if rotation fails
		Block[][] restore = input;

		// Go from Bottom to Top and then left to right rotate all the elements. This
		// algorithm has been emperically tested and works for all blocks that are
	...
	public Block[][] startRight(Block[][] input) {
		...
		// Go from right to left and then down up to rotate all the elements. This
		// algorithm has been emperically tested and works for all blocks in their
		// starting position
		// Without ever accidentally overwriting non moved blocks.
		for (int c = 0; c < 3; c++) {
			for (int r = 0; r < 3; r++) {
				if (currentElement.contains(temp[2 - r][2 - c]) && temp[2 - r][2 - c].isMarked() == false) {

					// Through practial tests or a simple check one can see that with this Algorithm
					// used on tetroids in the starting position no feel that a block gets copied to
					// Is allowed to be null, hence we can practically assert whether or not a
					// rotation is possible or not.
	...
	public void rotateRightI() {
		...
		if (horizontal) {
			...
			if (starting) {
				// The Block is in its starting position so we now know where all the other
				// elements are and need to be projected relative to the rotator element.
				...
				// The block is in the horizontal position where the rotator element is second
				// from left
			...
			// Determine in which vertical position the block is in. Left means that the
			// rotator element is 2nd from the bottom. Right means 3rd.
			Boolean left = true;
			...
			if (left) {
				// The Block is in the left position hence we can rotate it relative to the
				// rotator position
				...
				// Move the 2nd top Block because otherwise we would overwrite it if we first
				// moved the Rotator block
				panelField[rowtemp - 1][coltemp + 1] = panelField[rowtemp - 1][coltemp];
				panelField[rowtemp - 1][coltemp] = null;
				newpos = panelField[rowtemp - 1][coltemp + 1];
				newpos.moveRight();
				...
				// The Block is in the right position with the rotator second from the bottom.
				// Again we can exactly determine the location of our current and target
				// elements
				// Move the bottom most Block
				panelField[rowtemp + 1][coltemp - 2] = panelField[rowtemp + 2][coltemp];
				panelField[rowtemp + 2][coltemp] = null;
